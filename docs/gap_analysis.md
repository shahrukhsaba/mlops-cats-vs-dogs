...
|---|-------------|--------|----------|
| 1.1 | Git for source code versioning | ✅ Covered | `.git/` directory, [.gitignore](../.gitignore), full repo on GitHub |
| 1.2 | DVC for dataset versioning | ✅ Covered | [dvc.yaml](../dvc.yaml), [data/raw.dvc](../data/raw.dvc), [.dvc/](../.dvc/), [.dvcignore](../.dvcignore) |
| 1.3 | Track pre-processed data | ✅ Covered | DVC pipeline stage `prepare` outputs `data/processed/splits.json` |
| 1.4 | Pre-process to 224×224 RGB | ✅ Covered | [preprocess.py](../src/data/preprocess.py) — `load_and_resize_image()` resizes to (224, 224) RGB |
| 1.5 | 80/10/10 train/val/test split | ✅ Covered | [params.yaml](../params.yaml) defines 0.8/0.1/0.1; [config.py](../src/config.py) has constants |
| 1.6 | Data augmentation | ✅ Covered | [train.py](../scripts/train.py) — `TRAIN_TRANSFORMS_FULL` with flip, rotation, affine, color jitter |
| 1.7 | Baseline model (CNN) | ✅ Covered | [cnn.py](../src/model/cnn.py) — SimpleCNN (4-layer CNN with BatchNorm) |
| 1.8 | Save model as `.pt` | ✅ Covered | `torch.save(model.state_dict(), model_path)` in [train.py](../scripts/train.py); output → `models/model.pt` |
| 1.9 | MLflow experiment tracking | ✅ Covered | [train.py](../scripts/train.py) logs params, metrics (train_loss, val_loss, val_acc), confusion matrix, loss curves, model artifact |
...
| # | Requirement | Status | Evidence |
|---|-------------|--------|----------|
| 2.1 | REST API (FastAPI/Flask) | ✅ Covered | [api/main.py](../api/main.py) — FastAPI app |
...
| 2.4 | requirements.txt with version pinning | ✅ Covered | [requirements.txt](../requirements.txt) — all major libs pinned (torch==2.2.0, fastapi==0.109.2, etc.) |
| 2.5 | Dockerfile | ✅ Covered | [Dockerfile](../Dockerfile) — python:3.11-slim, copies src/api/models, runs uvicorn |
| 2.6 | Build and run locally, verify via curl | ✅ Covered | `docker build` & `docker run` instructions; [smoke_test.sh](../scripts/smoke_test.sh) verifies with curl |
...
| # | Requirement | Status | Evidence |
|---|-------------|--------|----------|
| 3.1 | Unit test for data pre-processing | ✅ Covered | [test_preprocess.py](../tests/test_preprocess.py) — 5 tests for `load_and_resize_image`, `get_train_val_test_splits`, `normalize_for_model` |
| 3.2 | Unit test for model/inference | ✅ Covered | [test_inference.py](../tests/test_inference.py) — 5 tests for `load_model`, `predict_proba`, `predict_label`, `preprocess_image` |
| 3.3 | Tests run via pytest | ✅ Covered | `pytest.ini` configured; CI runs `pytest tests/ -v` |
| 3.4 | CI pipeline (GitHub Actions) | ✅ Covered | [ci.yml](../.github/workflows/ci.yml) — triggers on push/PR to main/master |
...
| # | Requirement | Status | Evidence |
|---|-------------|--------|----------|
| 4.1 | Deployment target (K8s / Docker Compose) | ✅ Covered | **Both** options: [docker-compose.yml](../docker-compose.yml) + [k8s/](../k8s/deployment.yaml) (Deployment + Service) |
| 4.2 | K8s manifests (Deployment + Service) | ✅ Covered | `k8s/deployment.yaml` + `k8s/service.yaml` with health probes, resource limits |
| 4.3 | CD / GitOps flow | ✅ Covered | [cd.yml](../.github/workflows/cd.yml) — triggers after CI → pulls image → deploys → smoke tests |
...
| 5.4 | Prometheus + Grafana monitoring stack | ✅ Covered | [docker-compose-monitoring.yml](../monitoring/docker-compose-monitoring.yml), `prometheus.yml`, Grafana dashboards + datasources |
| 5.5 | Post-deployment performance tracking | ✅ Covered | [collect_predictions.py](../scripts/collect_predictions.py) — runs model on test batch, outputs `predictions_batch.json` with true labels, accuracy |
...
- **Comprehensive documentation** (README, [runbook](runbook.md), [gap_analysis](gap_analysis.md), [smoke_test_gap_analysis](smoke_test_gap_analysis.md), DEPLOYMENT, API, INDEX, VERIFICATION, Report)
